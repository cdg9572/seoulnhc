...
if($data_list_basic_cnt > 0) {
	for ($i=0; $i<$data_list_basic_cnt; $i++ ) {
            ...
            $jb_etc6					= $data_list_basic[$i]['jb_etc6'];
			...
			$jb_title 					= $C_Func->strcut_utf8($data_list_basic[$i]['jb_title'], 200, true, "...");	//제목 (길이, HTML TAG제한여부 처리)
        ...
        if($jb_type == "A" or $_SESSION['suserlevel'] == 9){       
		...
		*/
				...
				echo ("
                        <tr ${jb_tr}>
                            <td>${jb_no}</td>
                            <td class='title'>
                                <a href='" . $get_par . "' ${target}>${jb_title}</a>
                            </td>
