...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {
            ...
            $strSplit = explode('|', $jb_file_code);
            $thumbnail = $strSplit[0];

            $file_ext = substr( strrchr($thumbnail, "."),1); 
            $file_ext = strtolower($file_ext);	//확장자를 소문자로...

			// $img_src = '';
            // if($jb_file_code != '') {  
            //     if ($file_ext == "pdf" || $file_ext =="hwp" || $file_ext =="doc" || $file_ext =="xls" || $file_ext =="xlsx" ) {	
            //         $img_src = "<img src='/resource/images/thumb.jpg' alt='이미지 없음'  >";
            //     }
            //     else{
            //         $code_file = $GP->UP_IMG_SMARTEDITOR_URL. "/jb_${jb_code}/${thumbnail}";
			...
			if($jb_front_image != '') {
				...
				$img_src = "<img src='/resource/images/thumb.jpg' alt='이미지 없음' class='block' >";
			}
