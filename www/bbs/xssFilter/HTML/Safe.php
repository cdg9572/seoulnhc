    ...
    protected function writeAttrs($attrs)
    {
        if (is_array($attrs)) {
            foreach ($attrs as $name => $value) {
                ...
                $tempval = preg_replace_callback( '/&#x([a-f0-9]+);/mi', function ($m) { return chr(hexdec('0x'.$m[1])); }, $value); //"'
                $tempval = preg_replace_callback( '/&#x([a-f0-9]+);/mi', function ($m) { return chr(hexdec('0x'.$m[1])); }, $tempval);
