<?
	...
	function Main_Notice($jb_code, $limit) {
        ...
        ($jb_code == 20)?  $args['orderby']  = "order by A.jb_number DESC, A.jb_order asc, A.jb_idx desc" : $args['orderby']  = "order by A.jb_order asc, A.jb_idx desc"  ;  
		...
		for($i=0; $i<count($rst); $i++) {
			...
			if($jb_order < 100) {
				...
				$jb_notice = "<img style='margin-right:5px;transform:translateY(2px)' src='/resource/images/notice_icon2.png' alt='공지'>" ;
            }
            else{
                $jb_notice = "";
            }    
