...
CLASS ListClass
{
	...
	function setVals ($args) {
        ...
        $this -> conf_info['g_page'] 			= ($g_page)? $g_page : 1;
	...
	function listInfo ($args) {
        ...
        if ($_SERVER["REMOTE_ADDR"] == '183.102.91.158') {
	...
	function getCounts () {
        ...
        ($jb_code ==20 ) ? $startnum 	= ($g_page - 1) * $show_row : $startnum 	= ($page - 1) * $show_row;      
	...
	{		
		...
		if($total > 0)
		{			
			($g_page == 1)? $prev_page = $g_page : $prev_page = $g_page - 1 ;
			($g_page == 1)? $next_page = $g_page : $next_page = $g_page + 1 ;			
			$rst ='<a href="?g_page=1&' . $tail . '" title="처음으로 이동하기" class="btn prev"><span><img src="/resource/images/page-all-left.png" width="100%" alt=""></span></a> ';
			$rst .='<a href="?g_page='  . $prev_page . '&' . $tail . '" title="이전으로 이동하기" class="btn prev"><span><img src="/resource/images/page-left.png" width="100%" alt=""></span></a> ';
			for($i = $start ; $i <= $end ; $i++){
				$pageLastclass = "";
				if ($i>=10) {
					$pageLastclass = "class=\"page_Last\"";
				}				
				$rst .= ($i != $g_page)? "<a href='?g_page=" . $i . '&' . $tail . "' class='btn'>":"<strong class='btn current' title='현재 페이지'>";
				$rst .= ($i == $g_page)? "$i" : "$i";
				$rst .= ($i != $g_page)? "</a>":"</strong>";
				$rst .= ($i != $end) ? " " : "";
			}
			$rst .= ' <a href="?g_page=' . $next_page . '&'. $tail . '" title="다음 페이지 이동하기" class="btn next"><span><img src="/resource/images/page-right.png" width="100%" alt=""</span></a>';
			$rst .= ' <a href="?g_page=' . $this -> page_info['tpage'] . '&'. $tail . '" title="마지막 페이지 이동하기" class="btn next"><img src="/resource/images/page-all-right.png" width="100%" alt=""></a>';

		} else {			
			$rst ='<a href="?g_page=1&' . $tail . '" title="처음으로 이동하기" class="btn prev"><span><img src="/resource/images/page-all-left.png" width="100%" alt=""></span></a> ';
			$rst .='<a href="?g_page=1&'. $tail . '" title="이전으로 이동하기" class="btn prev"><span><img src="/resource/images/page-left.png" width="100%" alt=""></span></a> ';
			$rst .='<a href="?g_page=1&'. $tail . '" class="btn current">1</a>';			
			$rst .= ' <a href="?g_page=1&'. $tail . '" title="다음 페이지 이동하기" class="btn next"><span><img src="/resource/images/page-right.png" width="100%" alt=""</span></a>';
			$rst .= ' <a href="?g_page=1&'. $tail . '" title="마지막 페이지 이동하기" class="btn next"><img src="/resource/images/page-all-right.png" width="100%" alt=""></a>';

		}

		$this -> page_info['g_link'] = $rst;
	}
